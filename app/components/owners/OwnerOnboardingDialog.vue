...
import type { OwnerQuotaWindowDraft } from './OwnerOnboardingSelfBooking.vue'
...
import OwnerOnboardingSelfBooking from './OwnerOnboardingSelfBooking.vue'
...
export interface OwnerOnboardingDraft {
  ...
  annualCap?: number
  /** Seasonal quota windows applied after the owner is created. */
  quotaWindows: OwnerQuotaWindowDraft[]
}
...
function validateStep4(): boolean {
  const e: Record<string, string> = {}
  // A seasonal window can't request more nights than the annual cap.
  if (draft.value.annualCap && draft.value.annualCap > 0) {
    const over = draft.value.quotaWindows.find(w => w.maxNights > draft.value.annualCap!)
    if (over) {
      e.windows = `A quota window has ${over.maxNights} nights, but the annual cap is ${draft.value.annualCap}. Reduce the window below the cap.`
    }
  }
  for (const [i, w] of draft.value.quotaWindows.entries()) {
    if (!w.listingId || !w.startDate || !w.endDate) {
      e.windows = `Window ${i + 1} is missing a listing or dates.`
      break
    }
    if (w.startDate > w.endDate) {
      e.windows = `Window ${i + 1} has an end date before its start date.`
      break
    }
  }
  errors.value = e
...
async function handleSubmit() {
  ...
  try {
    const ownerPayload = {
      ...
      annualOwnerUseNightCap: draft.value.annualCap && draft.value.annualCap > 0
        ? draft.value.annualCap
        : undefined,
    }
    ...
    // Persist the seasonal quota windows configured in step 4.
    const ownerId = result.ownerId!
    for (const w of draft.value.quotaWindows) {
      const saved = upsertQuota({
        ownerId,
      ...
      if (!saved.success)
        toast.error(saved.error ?? `Failed to save quota window for ${w.listingId}.`)
    }
    ...
    <SheetContent class="flex w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
      ...
      <div class="flex items-center gap-2 border-b bg-muted/30 px-6 py-3 text-xs font-medium">
        ...
        <button
          type="button"
          class="flex items-center gap-1.5 rounded-full px-2 py-0.5 transition-colors"
          :class="step === 4 ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'"
          @click="step = 4"
        >
          <span
            class="flex size-4 items-center justify-center rounded-full text-[10px]"
            :class="step === 4 ? 'bg-primary-foreground text-primary' : 'bg-muted text-muted-foreground'"
          >4</span>
          Self-booking
        </button>
      </div>
      ...
      <div class="flex-1 min-h-0 overflow-y-auto px-6 py-5">
        ...
        />
        <OwnerOnboardingSelfBooking
          v-else
          :mappings="draft.mappings"
          :annual-cap="draft.annualCap"
          :windows="draft.quotaWindows"
          :errors="{ windows: errors.windows, annualCap: errors.annualCap }"
          @update:annual-cap="(v) => draft.annualCap = v"
          @update:windows="(v) => draft.quotaWindows = v"
        />
      </div>
