export type AlertType
  = | 'SMART_LOCK_DEAD'
    ...
    | 'DEPOSIT_DECISION_DUE'
    | 'DEPOSIT_DECISION_OVERDUE'
    | 'DEPOSIT_CHARGE_FAILED'
    | 'DAMAGE_CLAIM_RECORDED'
    | 'PARTNER_CLAIM_SUBMISSION_FAILED'
...
export const alertDisplayLabels: Record<AlertType, string> = {
  ...
  DEPOSIT_DECISION_DUE: 'Security Deposit - Charge or Close',
  DEPOSIT_DECISION_OVERDUE: 'Security Deposit - Decision Overdue',
  DEPOSIT_CHARGE_FAILED: 'Security Deposit - Card Charge Declined',
  DAMAGE_CLAIM_RECORDED: 'Damage Claim Recorded',
  PARTNER_CLAIM_SUBMISSION_FAILED: 'Insurance Claim - Submission Failed',
...
export const alertIcons: Record<AlertType, string> = {
  ...
  DEPOSIT_DECISION_DUE: 'i-lucide-credit-card',
  DEPOSIT_DECISION_OVERDUE: 'i-lucide-credit-card',
  DEPOSIT_CHARGE_FAILED: 'i-lucide-credit-card',
  DAMAGE_CLAIM_RECORDED: 'i-lucide-receipt-text',
  PARTNER_CLAIM_SUBMISSION_FAILED: 'i-lucide-shield-alert',
...
export const alertRouteMap: Partial<Record<AlertType, string>> = {
  ...
  DEPOSIT_DECISION_DUE: '/damage-protection',
  DEPOSIT_DECISION_OVERDUE: '/damage-protection',
  DEPOSIT_CHARGE_FAILED: '/damage-protection',
  DAMAGE_CLAIM_RECORDED: '/damage-protection',
  PARTNER_CLAIM_SUBMISSION_FAILED: '/damage-protection',
...
export function getDescription(type: AlertType, context: Record<string, any>): string {
  switch (type) {
    ...
    case 'DEPOSIT_DECISION_DUE':
    case 'DEPOSIT_DECISION_OVERDUE':
      return Number(context.charge_amount) > 0
        ? `${context.guest_name || 'Guest'}, ${context.currency || 'USD'} ${context.charge_amount} in claims to charge`
        : `${context.guest_name || 'Guest'}, no claims: close the deposit`
    case 'DEPOSIT_CHARGE_FAILED':
      return `${context.guest_name || 'Guest'}, ${context.reason || 'Card declined'}`
    case 'DAMAGE_CLAIM_RECORDED':
      return `${context.guest_name || 'Guest'}, ${context.currency || 'USD'} ${context.claim_amount || 0}`
    case 'PARTNER_CLAIM_SUBMISSION_FAILED':
    case 'PARTNER_CLAIM_REJECTED':
      return `${context.guest_name || 'Guest'}, ${context.reason || 'see the claim'}`
    case 'PARTNER_CLAIM_INFO_REQUESTED':
