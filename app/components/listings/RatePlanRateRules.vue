...
function updateRule(index: number, field: 'type' | 'value', newValue: string) {
  rules.value = rules.value.map((r, i) =>
...
const calculatedRate = computed(() => {
  ...
  for (const rule of rules.value) {
    ...
    if (rule.type === 'increase_by_amount')
      rate += val
    else if (rule.type === 'decrease_by_amount')
      rate -= val
    else if (rule.type === 'increase_by_percent')
      rate += rate * (val / 100)
    else if (rule.type === 'decrease_by_percent')
      rate -= rate * (val / 100)
  }
...
<template>
  <div class="space-y-3">
    ...
    <div v-if="rules.length === 0" class="rounded-lg border border-dashed bg-muted/30 p-4 text-center">
      <p class="text-sm text-muted-foreground mb-3">No modification rules. Parent rate will be used as-is.</p>
    ...
    <div v-else class="space-y-2">
      <div v-for="(rule, idx) in rules" :key="idx" class="flex items-end gap-2">
        <div class="flex-1 flex flex-col gap-1.5">
          ...
          <Select :model-value="rule.type" @update:model-value="(v) => updateRule(idx, 'type', String(v))">
        ...
        <div class="w-24 flex flex-col gap-1.5">
          ...
          <Input
            ...
            class="h-8"
            step="0.01"
            @update:model-value="(v) => updateRule(idx, 'value', String(v))"
          />
        </div>
