...
### Main Modules
- **Inbox** — Guest messaging system (4-panel layout) with Phone call tab, plus an **Internal** view: staff rooms per listing and role, right-click forward / create task on any message
...
- **Tenant Onboarding** — Register to activated dashboard: email verification, a 3 step wizard (Profile, Branding, Select plan), PMS model choice (`PMS_CM` / `PMS_ONLY` / `MIGRATION`), USD pricing with server validated promo codes, mock Stripe, then channel connect or Calry import, and a dashboard checklist
- **GM Dashboard** — Role-gated portfolio dashboard on `/` for `role-general-manager`, modelled on the shadcnblocks `dashboard14` hospitality block: four KPI tiles (occupancy tonight, ADR, RevPAR, room revenue), a bidirectional occupancy-flow chart, a daily revenue bar chart, a negative-guest-sentiment panel with per-row action-needed handling, and a bookings panel with a date-strip navigator.
...
- **Upsell Smart Lock Access PRD** → `docs/superpowers/specs/2026-09-09-upsell-smart-lock-access-prd.md`
