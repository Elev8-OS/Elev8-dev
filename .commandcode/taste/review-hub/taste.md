...
- Guest/booking status uses a fixed semantic color scheme on cards across the app: Grey = Unverified (guest hasn't filled info), Green = Verified (info filled, look for "Thank you for filling your guest guide" message), Orange = Checked-in, Blue = Checked-out, Grey with strikethrough line = Cancelled, Black = Manual Block (calendar block, not a reservation — has a reason), Yellow = Inquiry (not yet confirmed, awaiting payment; check for auto-remove setting after XX hours). Confidence: 0.92
